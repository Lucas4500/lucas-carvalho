<div class="card mx-auto my-5 col-md-6">
    <div class="card-body">
        <form action="<?= $action ?>" method="POST">
            <p class="h4 text-center py-4">Editar Custo Operacional</p>
            <?= $info ?>
            ...
            <input value="<?= $data->nome_do_custo ? $data->nome_do_custo : '' ?>" type="text" id="nome" name="nome" class="form-control" max-length="30" required>
            <br>
            <label for="descricao" class="grey-text font-weight-light">Descrição</label>
            <input value="<?= $data->descricao ? $data->descricao : '' ?>" type="text" id="descricao" name="descricao" class="form-control" max-length="150" required>
            <br>
            <div class="text-center py-4 mt-3">
                <button class="btn btn-outline-blue" type="submit">Editar<i class="fa fa-paper-plane-o ml-2"></i></button>
            </div>
        </form>
    </div>
</div>